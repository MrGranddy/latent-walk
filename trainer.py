...
class Trainer:
    ...
    def create_walk_grid(self, index):
        ...
        with torch.set_grad_enabled(False):
            ...
            for part_idx in range(num_parts):
                ...
                diff_ws = self.mapping((c3, p2, p1))
                walked_diff_ws = self.walker(diff_ws, walks[part_idx, ...].view(-1), eps.view(-1))
                ws = walked_diff_ws + self.mapping.latent_avg.unsqueeze(0)
    ...
    def train_step(self, index):
        ...
        with torch.set_grad_enabled(True):
            ...
            diff_ws = self.mapping((c3, p2, p1))
            walked_diff_ws = self.walker(diff_ws, walks, eps)
            ws = walked_diff_ws + self.mapping.latent_avg.unsqueeze(0)
